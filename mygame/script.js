...
async function loadData() {
  try {
    const keys = [
      'stars', 'collection', 'activeBattleNft', 'totalStarsEarned',
      'battleHistory', 'referralCode', 'referredFriends', 'starsFromReferrals'
    ];
    const values = await Promise.all(keys.map(key => getCloudItem(key)));
    
    stars = values[0] ? parseInt(values[0]) : 100;
    collection = values[1] ? JSON.parse(values[1]) : [];
    activeBattleNft = values[2] ? JSON.parse(values[2]) : null;
    totalStarsEarned = values[3] ? parseInt(values[3]) : 0;
    battleHistory = values[4] ? JSON.parse(values[4]) : [];
    referralCode = values[5] || null;
    referredFriends = values[6] ? JSON.parse(values[6]) : [];
    starsFromReferrals = values[7] ? parseInt(values[7]) : 0;
  } catch (error) {
...
// Debounced load for visibilitychange
